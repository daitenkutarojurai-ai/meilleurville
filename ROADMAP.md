...
Item 2 du plan agent « vacances monoparentales ». La série était à parité 22 FR / 22 EN depuis le
batch EN qui a suivi le lot du 02/09, donc écart nul : la main revenait au FR. **+8 guides, compteur
mesuré `grep -c 'slug: "vacances-monoparentales-'` = 30 ; `GUIDES` 1 097 → 1 105.**
`npm run search-index` relancé (`data/search-index.json` 1 105 guides, **265 tags** contre 264 —
un seul tag franchit le seuil de 3 guides, `gare-tgv-excentree`, porté à 3 par Avignon aux côtés de
Mâcon et d'Aix-en-Provence), d'où le passage de `npm run sitemap:check` : FR 29 194 → **29 203 URL**, soit
exactement les 8 guides plus la page de tag, chaque URL déclarée ayant une page et réciproquement.
`npm run parity` (code 0) et `npm run hreflang:check` repassés, verts.

**La règle de sélection du batch 3 a été recalculée et non recopiée**, comme les batches 3 et 4 de la
série célibataire l'ont établi : la règle « arriver en TGV/RER **et** circuler sur place en métro,
tram ou BHNS, ou à défaut transport ≥ 6,8 » de `MonoparentalExtras.tsx` retient toujours **53 villes
sur 540**, dont **32 non couvertes**. Après retrait des communes d'Île-de-France (motif inchangé
depuis le batch 2 : une excursion depuis Paris n'est pas un séjour, et leur axe coût à 2,2 mesure un
marché résidentiel), il reste **12 candidates**, par fit décroissant : Reims, Le Mans, Lille et
Mulhouse à 5,8, Saint-Étienne et Angoulême à 5,6, Montpellier à 5,5, Avignon à 5,4 — puis Nice à
5,3, Toulon à 5,0, Perpignan à 4,9 et Marseille à 4,2. **Les quatre paliers sont pris entiers et on
s'arrête avant Nice**, ce qui donne 8, le haut de la fourchette de 6 à 8 : la convention de
`lib/owner-rankings.ts` interdit de couper une égalité en son milieu, et le palier à 5,8 en compte
quatre à lui seul.

**Thèse du batch : c'est le premier lot où l'axe le plus lourd du profil décroche, et chaque guide
dit ce qui compense.** Le composite monoparental pondère la sécurité à **0,30**, devant transports
et coût à 0,25 : descendre le classement revient donc mécaniquement à descendre la sécurité.
**Sept des huit destinations sont sous la médiane de sécurité des trente, qui vaut 5,3 ; la
huitième, Le Mans, l'atteint tout juste**, et **les deux scores les plus bas de toute la série sont
dans ce lot** (Montpellier 3,7, Lille 3,9). Ce qui les fait entrer malgré cela : **Lille** porte le
3ᵉ score de transports des trente (8,7, derrière Strasbourg et Lyon à 9,0) et deux gares distantes
d'environ 500 m ; **Saint-Étienne** porte le **meilleur score de coût des trente** (7,1) avec
**770 € le T3 et 1 500 € le mètre carré, les deux minimums de toute la série** (source :
`data/housing.ts`) ; **Avignon** porte la **meilleure culture des trente** (9,0, à égalité avec
Strasbourg et Lyon) ; **Mulhouse** offre la 2ᵉ valeur la plus basse de la série sur le T3 comme sur
le mètre carré. Symétriquement, chaque guide nomme aussi son plancher : **Lille** ferme le classement
de nature des trente (3,4) et **Reims** l'avant-dernière place (4,2) ; **Saint-Étienne** ferme celui
de la qualité de vie (4,9) **et** celui de la culture (5,9) ; **Angoulême** ferme celui des
transports (5,2).

**Trois vérifications faites avant rédaction, et qui ont chacune changé un guide.**
① **Le tramway de Montpellier est gratuit, mais pas pour un visiteur.** La gratuité du réseau, très
relayée depuis décembre 2023, est **réservée aux habitants de la métropole** et s'exerce au moyen
d'un **pass nominatif** délivré sur pièce d'identité, photographie et justificatif de domicile ; un
non-résident achète des titres à la durée ou un abonnement. C'est la règle générale déjà posée au
glossaire (« la gratuité d'un réseau est réservée aux résidents »), et un parent qui bâtit un budget
de séjour sur l'inverse se trompe de ligne. Le guide le corrige **en première section**.
② **Le festival de bande dessinée d'Angoulême n'a pas eu lieu en 2026.** L'édition qui devait se
tenir **du 29 janvier au 1ᵉʳ février 2026 a été annulée** — première annulation depuis la création du
festival en **1974**, hors celle de 2021 due à l'épidémie — un événement alternatif gratuit s'étant
tenu aux mêmes dates, et une reprise étant attendue en **2027**. Un guide daté 2026 bâti sur ce
festival n'aurait envoyé personne nulle part ; le guide dit l'annulation, renvoie les dates 2027 à
l'organisateur, et s'appuie sur la **Cité internationale de la bande dessinée et de l'image**, qui
elle est permanente. Même précédent que l'Hermione absente de Rochefort (batch 36 tourisme) et que
le festival de cerf-volant de Dieppe, biennal et impair.
③ **Angoulême n'a pas de funiculaire.** La première source consultée l'affirmait ; elle décalque le
funiculaire de **Pau**, établi au batch 3 de cette même série. Ce qui existe réellement est le
réseau **Möbius**, deux lignes de bus à haut niveau de service et **deux navettes électriques
gratuites** reliant le plateau du vieil Angoulême aux quartiers bas, la gare étant par ailleurs
équipée d'ascenseurs et d'escaliers mécaniques vers la passerelle et les quais. **Une affirmation
vraisemblable et mal sourcée se retire, elle ne s'arrondit pas.**

Autres faits vérifiés en ligne avant rédaction et qui portent leurs guides : **métro de Lille mis en
service le 25 avril 1983**, premier métro entièrement automatique et sans conducteur au monde
(système VAL conçu par une équipe universitaire de Villeneuve-d'Ascq), première ligne de **9 km et
13 stations**, gares **Lille-Flandres et Lille-Europe à ~500 m** à pied et stations voisines de la
même ligne ; **enceinte gallo-romaine du Mans** du début du **IVe siècle**, quadrilatère de
**450 × 200 m** soit **1 300 m de tour**, ~**500 m** de murs visibles et **11 tours sur 26**, la
mieux conservée de l'Empire avec Rome et Constantinople — c'est-à-dire, en logistique de parent solo,
**une boucle d'un kilomètre trois qu'un enfant termine sans être porté** ; **Cité de l'Automobile**
de Mulhouse, plus grand musée automobile du monde, ~**430 véhicules** dont ~**150 Bugatti**, et
**Cité du Train**, plus grand musée ferroviaire d'Europe, **60 000 m²** et **138 pièces** de matériel
roulant, devenue Cité du Train en **2005** ; **tramway de Saint-Étienne en service sans interruption
depuis le 4 décembre 1881** (première ligne de 5,5 km, La Terrasse–Bellevue), le plus ancien réseau
de France, et **ville créative UNESCO de design depuis le 22 novembre 2010**, la seule française à ce
titre ; **cathédrale de Reims inscrite à l'UNESCO en 1991** avec Saint-Remi et le palais du Tau,
gothique du XIIIe, **29 rois sacrés**, **tramway ouvert en avril 2011** dont la ligne A dessert la
gare puis passe à proximité de la cathédrale ; **centre historique d'Avignon inscrit en 1995**,
**Palais des papes** plus important palais gothique du monde avec ~**15 000 m²** de plancher.

⚠️ **La gare excentrée d'Avignon est le contre-exemple abouti du piège documenté aux batches 2 et 3**,
et c'est l'angle du guide. Avignon TGV est bien à ~**6 km** du centre, dans le secteur de la
**Courtine** — soit la configuration de Mâcon-Loché à 7 km et d'Aix TGV à une quinzaine — mais depuis
le **15 décembre 2013** le **raccordement de la Courtine**, connu régionalement comme la **virgule
d'Avignon**, relie cette gare à Avignon-Centre avec une **trentaine d'allers-retours quotidiens** et
un trajet d'**environ cinq minutes**. C'est la meilleure arrivée de toute la série pour une gare
excentrée, et elle n'a de valeur que si le lecteur la connaît avant de réserver un taxi.

⚠️ **Deux limites de nos propres données mesurées ce run et écrites dans les guides concernés, pas
seulement ici.**
- **L'indicateur d'affluence classe Avignon au palier calme en juillet, et c'est faux.**
  `crowdednessForMonth()` n'a que **trois profils annuels distincts sur les trente destinations** de
  la série, et il place Avignon au palier bas — le même que Reims, Le Mans, Mulhouse, Saint-Étienne
  et Angoulême — donc à **3 sur 5** en juillet. Or le seul festival **off**, dont la **60ᵉ édition
  s'est tenue du 4 au 25 juillet 2026**, a programmé **1 780 spectacles**, soit environ **1 250
  représentations par jour**, dans une commune de **91 760 habitants**, en plus du festival officiel.
  Le mécanisme est le même qu'au Mans, où l'indicateur ne voit pas le week-end de course de juin :
  **il raisonne en régime moyen et est structurellement aveugle à un événement.** Ne pas en déduire
  qu'il faut le recalibrer sur des événements, mais **le dire là où il ment**.
- **Le plafond de collecte des parcs touche 22 des 30 destinations de la série**, dont six des huit
  de ce lot (Reims, Le Mans, Lille, Mulhouse, Saint-Étienne, Montpellier) : `PARKS_PER_CITY` vaut 40,
  leur compte est un **plancher** et ne se compare pas à **Angoulême (19)** ni à **Avignon (29)**, qui
  publient un total réel. Les guides concernés le disent.

⚠️ **Vingt-six affirmations fausses au premier jet, corrigées avant commit.** Même mode de défaillance
qu'aux batches précédents et qu'au palmarès de novembre : les scores pris un à un étaient justes, ce
sont **les rangs, les égalités et les mois** qui dérapaient. La plus lourde touchait les huit guides
à la fois : **la médiane de sécurité des trente vaut 5,3 et non 5,4**, ce qui rendait littéralement
faux le « les huit sont toutes sous la médiane » qui portait la thèse du batch — Le Mans, à 5,3,
l'atteint exactement. Ensuite, six rangs pris **au milieu d'une égalité** (Le Mans 8ᵉ en coût dans une
égalité de quatre, Mulhouse idem, Lille 7ᵉ en culture dans une égalité avec Toulouse, Angoulême 2ᵉ en
coût à égalité avec Valence, Avignon 18ᵉ en coût à égalité avec Rennes, Le Mans 27ᵉ en transports dans
une égalité de quatre) ; quatre **égalités énumérées incomplètes** (Pau oubliée deux fois au palier
6,1 en transports, Nancy oubliée au palier 7,6 en culture, Vannes et Poitiers oubliées au palier 6,2,
Reims présentée comme seule 2ᵉ du lot en sécurité alors qu'Angoulême l'égale) ; **le palier d'affluence
4 placé en octobre au lieu de septembre** sur Lille et Montpellier ; **Montpellier donnée 26ᵉ en coût
au lieu de 25ᵉ** ; **Mulhouse donnée 2ᵉ du lot en transports derrière deux villes** ; et quatre
erreurs de relevé climatique (août de Lille annoncé pas plus sec que mai alors qu'il l'est d'un jour ;
septembre d'Angoulême donné à 8 jours de pluie alors qu'il est à 10 ; les villes du nord du lot
données à 9-14 jours de pluie en juillet-août alors qu'elles sont à 7-10 ; la fenêtre d'Avignon donnée
à 14-21 °C alors qu'elle est à 17-21). **Le contrôle qui les a trouvées est un tri complet des trente
villes sur chaque axe cité, avec impression des égalités, pas une relecture.**

Contrôles passés : `npx tsc --noEmit` **propre**, `npm run integrity` (guides FR 1 097 → 1 105),
`search-index` + `search-index:check`, `sitemap:check`, `parity`, `hreflang:check`, plus la
vérification que les 8 guides sont **retrouvés par `guideCityPhoto()`** avec leur photo d'en-tête,
**listent leur propre ville dans `relatedCities`** et sont **remontés par la recherche inverse** sur
leur page ville. `metaTitle` 36-44 caractères, `metaDesc` 140-148, 7 sections par guide, densité
d'accents 0,156-0,178 par mot (seuil ascii-strip 0,09), **0 em-dash** sur les huit, aucun `m2` /
`EUR` / `deg` ascii, aucun mojibake. `npm run build` **non lancé, volontairement**
(cf. CLAUDE.md § Commands depuis le batch 27).

**Prochain run : batch EN** (`single-parent-holidays-[city]-2026`, +8), l'écart FR→EN étant de 8
villes — reims, le-mans, lille, mulhouse, saint-etienne, angouleme, montpellier, avignon. Aucun slug
à arbitrer : les huit slugs de seed s'écrivent tels quels, et la règle du batch 33 de la série
tourisme (**côté EN le slug se dérive du slug de seed tel quel**) n'a rien à trancher — `le-mans`
garde son article comme `le-cannet` et `le-tampon`. Quatre points de vigilance pour ces jumelles :
① **la correction du tramway gratuit de Montpellier vise encore plus juste en anglais**, un visiteur
étranger étant le plus exposé à lire « free tram » sans la condition de résidence, et la même règle
vaut pour Dunkerque et les ~40 réseaux gratuits français ; ② **Lille a de la matière propre à l'angle
anglophone** que le FR ne porte pas — Eurostar, la proximité de Londres et de Bruxelles, et le fait
que le VAL lillois est l'ancêtre direct des métros automatiques que le lecteur connaît chez lui ;
③ **l'enceinte du Mans se compare à celles de Rome et de Constantinople**, ce qui est le meilleur
point d'entrée pour un lecteur qui ne sait pas situer la Sarthe ; ④ **le festival off d'Avignon est
connu du monde anglophone sous le nom d'Avignon Festival**, et l'annulation d'Angoulême comme la
reprise attendue en 2027 doivent être écrites comme des faits datés, pas comme une réserve vague.

Pour un batch FR **ultérieur**, il reste **4 candidates** de la règle hors Île-de-France (Nice 5,3,
Toulon 5,0, Perpignan 4,9, Marseille 4,2), trop peu pour un lot et de plus en plus bas sur l'axe que
le profil pondère le plus : c'est probablement le signal que la série FR approche de sa fin utile, et
que le prochain vrai gap de la verticale est ailleurs (miroir EN du hub enrichi
`/vacations/profile/monoparental`, qui reste le gabarit générique).
...
Item 2 du plan agent « vacances célibataire ». La série comptait 36 guides FR et 36 jumelles EN
depuis le batch EN du 11/09 (`solo-travel-in-` refermée à 36/36), donc écart nul : la main revenait
au FR. **+7 guides, compteur mesuré `grep -c 'slug: "vacances-celibataire-'` = 43 ; `GUIDES`
1 112 → 1 119.** `npm run search-index` relancé (`data/search-index.json` 1 119 guides, 266 →
**267 tags** : `week-end célibataire Nouvelle-Aquitaine` franchit le seuil de 3 guides et crée
`/tags/week-end-celibataire-nouvelle-aquitaine`), d'où le passage de `npm run sitemap:check`
(FR 29 212 → **29 220 URL**, soit les 7 guides plus la page de tag ; EN 28 800 inchangé par ce run).
Contrôle de photo et de remontée passé **sur les 43 guides de la série** : 43/43 retrouvés par
`guideCityPhoto`, 0 orphelin, 0 collision, chacun des 7 neufs listé sur sa page ville.

⚠️ **C'est le run où la règle anti-station-fantôme franchit son propre seuil, et elle ne peut pas
servir au batch 7 telle quelle.** La règle du batch 3 a été recalculée et non recopiée, comme les
batches 4 et 5 l'imposent (part des 15-29 ans Insee 2022, `pop2022 ≥ 60 000`, `culture ≥ 6,0`,
`life ≥ 5,5`, hors banlieues de métropoles déjà couvertes et hors villes déjà traitées). Elle rend
18 villes éligibles, dont 8 écartées comme banlieues franciliennes ou lyonnaises au même titre que
Villeurbanne depuis le batch 3 (Villeurbanne 29,2 %, Ivry-sur-Seine 25,1 %, Levallois-Perret,
Versailles, Courbevoie, Boulogne-Billancourt, Issy-les-Moulineaux, Rueil-Malmaison). Il reste
**exactement dix candidats réels**, dont les sept premiers : **Saint-Pierre de La Réunion 19,52 %
(199ᵉ), Colmar 19,43 % (209ᵉ), Annecy 19,41 % (212ᵉ), Niort 18,96 % (236ᵉ), Nice 18,44 % (268ᵉ),
Montauban 17,84 % (301ᵉ), Fort-de-France 15,80 % (420ᵉ)**. La coupe se fait sur un écart franc,
Fort-de-France 15,7994 % contre Antibes 14,5186 %, donc aucun palier n'est coupé en son milieu.
Mais la dispersion des lots successifs raconte l'essentiel : **b3 36,36 → 28,93 %, b4 28,80 →
22,69 %, b5 21,72 → 19,73 %, b6 19,52 → 15,80 %**, et ce lot est le premier dont **deux villes
passent sous la médiane nationale de 18,4 %** (Montauban 17,84, Fort-de-France 15,80, cette
dernière étant la plus basse des 43). La série a consommé les villes étudiantes. Appliquer la règle
mécaniquement au batch 7 reviendrait à publier sous la bannière « villes vivantes hors saison » des
communes que la mesure elle-même ne soutient plus : **le batch 7 doit nommer un autre mécanisme,
ville par ville, avant de choisir.**

C'est d'ailleurs ce que fait ce lot, et c'est sa colonne vertébrale éditoriale : **la mesure change,
la question ne change pas.** La vitalité hors saison vient ici de l'économie de semaine (Niort et
ses **plus de dix mille emplois** MAIF, MACIF, MAAF et SMACL ; Montauban préfecture ; Fort-de-France
capitale administrative et portuaire), de la taille brute (Nice, 353 701 habitants, 4ᵉ ville de la
série), d'un calendrier inversé (Colmar, Saint-Pierre) ou d'un équipement neuf (Annecy). Chaque
guide nomme **son** mécanisme au lieu de s'abriter derrière un proxy qui ne le porte plus. Deux
villes du lot **perdent des habitants**, ce qui est une première pour la série : Fort-de-France
−7,2 % depuis 2016, la plus forte contraction des 43, et Colmar −3,6 %, la troisième.

**Les trois faits qui portent les guides et qu'aucun guide du site ne portait encore.** ① La **Cité
internationale du cinéma d'animation a ouvert le 19 juin 2026** au Haras d'Annecy (site du XIXᵉ
réhabilité, parc de 2,6 ha) : musée permanent, cinéma, expositions et résidences, exploitée par
CITIA, plus de **8 000 œuvres** venues du Château-Musée, et surtout **ouverte 362 jours par an**.
C'est la différence entre un événement d'une semaine (le festival, 50ᵉ édition du 21 au 27 juin
2026, Mifa du 23 au 26, 16 000 professionnels et 160 000 visiteurs) et un équipement, donc la seule
réponse solide à la question que cette série pose à Annecy depuis le début. Vérifié qu'aucun guide
FR ni EN ne la mentionnait. ② **Colmar est la seule destination de la série dont la pointe est en
hiver** : six marchés de Noël du **23 novembre au 29 décembre 2026**, plus de 180 chalets,
**1,6 million de visiteurs** à l'édition précédente dans une commune de 67 360 habitants, soit
exactement le mois que les 36 guides précédents recommandent comme creux. La consigne de calendrier
de la série s'y inverse, et la consigne de budget avec elle. ③ **Fort-de-France n'est pas une
station** : sa scène nationale **Tropiques Atrium** (fusion CMAC / L'Atrium en 2015, salle
Aimé-Césaire 958 places, salle Frantz-Fanon 276) programme **environ 160 spectacles par saison**,
densité que peu de communes de 75 000 habitants atteignent en métropole, et c'est ce chiffre et non
la démographie qui justifie sa présence ici.

🔧 **Correctif de données : `lib/transit.ts` rendait `{}` pour Colmar et Montauban, qui ont toutes
deux un TGV direct.** La table documente `{}` comme « inconnu » et non « pas de desserte », et les
surfaces le respectent (elles retombent sur le résumé par score et n'affirment rien). Mais **un
rédacteur lit une absence comme une absence** : c'est précisément le piège dans lequel le batch 5
a failli tomber en comptant « les villes de la série sans TGV ni mode urbain sur rails », et le
contrôle relancé ce run sur les 43 slugs l'a rendu visible. Vérifié avant écriture : Colmar est
reliée à **Paris-Est par un TGV inOui direct**, deux allers-retours quotidiens, 2 h 20 au plus
rapide ; **Montauban-Ville-Bourbon** est desservie par TGV inOui et Ouigo sur l'axe
Paris-Bordeaux-Toulouse, de l'ordre de 3 h 55 depuis Montparnasse, plus des Intercités
Paris-Vierzon-Limoges-Toulouse dont une relation de nuit. Les deux entrées sont ajoutées avec
`tgv: true` seulement ; `bhns` et `velo` restent non renseignés **faute de vérification et non par
constat d'absence**, et le commentaire du fichier le dit. ⚠️ Corollaire de méthode pour les
prochains batches : **ne jamais dériver une affirmation négative d'une fiche `{}`** ; le contrôle
transit de la série doit séparer « fiche vide » de « fiche renseignée sans le mode », ce qu'il fait
désormais (5 fiches vides : troyes, bourges, saint-pierre-reunion, colmar avant correctif,
montauban avant correctif).

⚠️ **Huit affirmations fausses écrites au premier jet et corrigées avant commit — la parade a
rattrapé le lot une quatrième fois de suite.** Comme aux batches 3, 4 et 5, les scores *de la ville
...
les voir. Le remède reste **sortir la matrice des axes du lot avec ses paliers d'ex æquo et
vérifier chaque superlatif contre elle**, avant relecture. Les huit :
- **Montauban** : « Toulouse est la ville la plus jeune de ce corpus **après Lille et Nancy** ».
  Elle est **sixième** des 42 communes de plus de 100 000 habitants du corpus (32,60 %), derrière
  Lille 36,85, Nancy 36,36, Rennes 34,14, Rouen 33,37 et Caen 33,32.
- **Montauban** donnée « à une **cinquantaine de minutes** de Toulouse en train », deux fois : le
  TER met **environ trente-cinq minutes** pour quarante-six kilomètres.
- **Montauban** : Sapiac annoncé « **trente** euros de moins que Villebourbon » alors que 620 − 560
  = **60**. Même famille que l'écart de sept siècles et demi lu « deux siècles et demi » à Sens
  (batch 37) et que les « trois points et demi » d'Avignon au batch 5 : les deux nombres sont
  justes, c'est la **soustraction** qui dérape.
- **Montauban** présentée comme « la **première** destination de la série sous la médiane nationale
  des 15-29 ans » : Fort-de-France sort le même jour et est **plus basse**.
- **Montauban** : « novembre **et décembre** sont le vrai creux, à la fois le plus arrosé et le
  moins lumineux » — décembre compte 8,7 jours de pluie contre 10,0 en novembre et 9,2 en janvier.
- **Niort** : « le creux court de novembre **à février**, sous trois heures de soleil par jour » —
  février est à **3,7 h/j**, au-dessus du seuil annoncé dans la même phrase.
- **Colmar** : mai et juin donnés « avec **presque sept heures** de soleil quotidiennes » — 6,7 h en
  mai mais **7,5 h** en juin, donc davantage.
- **Nice** : 18,4450 % annoncés « **exactement** la médiane nationale » — c'est une coïncidence
  d'arrondi, la phrase dit désormais « au dixième près ».

⚠️ **Quatre des sept villes ont une station climatique trop éloignée pour que ses normales décrivent
la ville, et les guides le disent plutôt que de publier le chiffre.** C'est une réserve que la série
n'avait jamais formulée, et elle est ici décisive : Nice 7 km et Fort-de-France 2 km sont fiables,
Montauban 43 km (Toulouse-Blagnac, même plaine) est acceptable, mais **Colmar est rattachée à
Strasbourg-Entzheim à 56 km** alors qu'elle est abritée par les Vosges et compte parmi les villes
les plus sèches de France, **Annecy à Grenoble-Saint-Geoirs à 86 km** dans une autre vallée et sans
effet de lac, **Niort à Nantes-Bouguenais à 126 km**, et surtout **Saint-Pierre à Saint-Denis-Gillot
à 50 km, sur l'autre côte de l'île**. Ce dernier cas est chiffré et sans appel : la côte sous le vent,
du Port à Saint-Pierre, reçoit **moins de 1 000 mm par an** quand la côte au vent dépasse **3 000 mm**,
Saint-Denis se situant autour de **1 700 mm**. Publier les 13,8 jours de pluie de janvier relevés à
Gillot comme s'ils décrivaient Saint-Pierre serait faux d'un facteur considérable : le guide ne
publie donc **aucune normale mensuelle** pour Saint-Pierre, et Colmar n'affiche **aucun chiffre de
pluie**. Même doctrine que le FPU en ALD du glossaire et que le décompte de la crue de Vaison
(batch 46) : une phrase sans chiffre vaut mieux qu'un chiffre faux.

**Autres pièges de localisation et de calendrier écartés avant rédaction**, convention « accessible
depuis » tenue partout : ① le **Théâtre Luc Donat est au Tampon**, rue Victor Le Vigoureux, et non à
Saint-Pierre, alors que les programmes touristiques le rattachent régulièrement aux sorties
saint-pierroises ; ② le **réseau urbain de Saint-Pierre est Alternéo (CIVIS)**, à ne confondre ni
avec **Estival**, qui est celui de l'Est, ni avec **Citalis**, celui du Nord cité par le guide
Saint-Denis du batch 5 ; ③ le **Marais poitevin commence à Coulon**, commune distincte à une dizaine
de kilomètres de Niort ; ④ le **carnaval de Nice 2026 s'est tenu du 11 février au 1er mars** et est
cité au passé comme le Festival de Cornouaille l'était au batch 5, une page publiée en septembre ne
promettant pas une édition déjà passée ; ⑤ le projet de **nouvelle salle SMAC à Montauban** est
écarté de la copie faute de date d'ouverture vérifiable, seul le Rio Grande en activité est cité ;
⑥ la capacité du **Grillen** est donnée en ordre de grandeur, les sources divergeant entre 330
places et deux salles de 350 et 180.
...
FR 1 112 → 1 119), `search-index` + `search-index:check`, `sitemap:check`, `npm run parity`
...
`city-population`, de `housing`, de `neighborhoods` et des normales, seuls restant les nombres
vérifiés en ligne et listés dans ce paragraphe) et une vérification d'encodage (densité d'accents
0,149-0,163 **par mot** contre un seuil ascii-strip de 0,09, **0 em-dash** sur les sept réunis,
aucun `m2` / `EUR` / `deg` ascii, aucun mojibake, aucune guillemet courbe).

**Écart FR→EN après ce batch : 7 villes** (nice, annecy, colmar, saint-pierre-reunion, niort,
montauban, fort-de-france) — au-dessus du seuil de ~6, donc **le prochain run doit être un batch
EN** de la série `solo-travel-in-[city]-2026`. Rappel de la règle du batch 33, qui vaut aussi pour
cette série : côté EN le slug se dérive du **slug de seed tel quel**, donc
`solo-travel-in-saint-pierre-reunion-2026` garde son suffixe de désambiguïsation et
`solo-travel-in-fort-de-france-2026` sa forme complète ; aucun des sept ne porte d'article, donc
rien d'autre à arbitrer. Quatre points de vigilance pour ces jumelles : ① **Nice, Annecy et
Fort-de-France ont de la matière propre à l'angle anglophone** que le FR ne porte pas — Nice a une
histoire de villégiature britannique inscrite jusque dans le nom de la Promenade des Anglais,
Annecy est un rendez-vous professionnel mondial de l'animation une semaine par an, et la Martinique
se lit depuis les Caraïbes anglophones ; ② **les DROM ne sont pas dans Schengen** et la Martinique
est **hors territoire TVA et accises de l'UE**, rappels déjà posés par le batch 33 et à reprendre
tels quels ; ③ **La Réunion est à UTC+4 sans heure d'été et ses saisons sont inversées**, ce que le
guide FR de Saint-Pierre porte déjà ; ④ la réserve sur les stations climatiques doit être reprise,
pas allégée : c'est le fond du guide Saint-Pierre.

Pour le batch FR **suivant**, il ne reste que **trois candidats** de la règle actuelle
(Antibes 14,52 %, Cannes 14,43 %, Ajaccio 13,70 %), tous trois nettement sous la médiane nationale
et tous trois des stations balnéaires, c'est-à-dire exactement le profil que cette série existe pour
distinguer. **Ne pas les publier au motif que la règle les classe** : soit le batch 7 change de
mécanisme (économie de semaine, taille, calendrier inversé, équipement à l'année, comme ce lot le
fait déjà ville par ville), soit il change de critère cardinal. Les mesures publiées jamais
mobilisées par cette verticale et disponibles au besoin : `lib/city-income.ts` (Filosofi, 533/540),
`lib/property-prices.ts` (DVF, 499/507) et la structure d'âge complète de `lib/city-population.ts`.

### F61 — série célibataire, batch 5 : `vacances-celibataire-[ville]-2026` (2026-09-05)

Item 2 du plan agent « vacances célibataire ». La série comptait 29 guides FR et 29 jumelles EN
depuis le batch EN du 23/08 (`solo-travel-in-` refermée à 29/29), donc écart nul : la main
revenait au FR. **+7 guides, compteur mesuré `grep -c 'slug: "vacances-celibataire-'` = 36 ;
`GUIDES` 1073 → 1080.** `npm run search-index` relancé (`data/search-index.json` 1 080 guides,
**257 tags, inchangé** — aucun tag neuf n'a franchi le seuil de 3 guides, donc aucune page
`/tags/` créée), et `npm run sitemap:check` repassé quand même : FR 29 162 → **29 169 URL**, soit
exactement les 7 guides neufs ; EN 28 737 inchangé. Les 7 guides sont vérifiés **retrouvés par
`guideCityPhoto` et pourvus de leur photo d'en-tête** après écriture.

**La règle du batch 3 a de nouveau été recalculée et non recopiée**, comme le batch 4 l'avait
imposé après le rattrapage d'Orléans. Réappliquée (part des 15-29 ans Insee 2022, `pop2022 ≥ 60 000`,
`culture ≥ 6,0`, `life ≥ 5,5`, hors banlieues de métropoles déjà couvertes et hors villes déjà
traitées), elle confirme cette fois la liste annoncée au batch 4 sans surprise. Ordre retenu et rangs
nationaux sur la valeur **non arrondie** (538 villes mesurées, médiane 18,4 %) : **Le Mans 21,7 %
(95ᵉ), Valence 21,4 % (106ᵉ), Saint-Denis de La Réunion 21,1 % (120ᵉ), Mulhouse 20,8 % (134ᵉ),
Avignon 20,3 % (158ᵉ), Bourges 20,3 % (162ᵉ), Quimper 19,7 % (190ᵉ)**. Deux points de convention.
Le **palier Avignon / Bourges à 20,3 %** (20,3487 % et 20,2983 % non arrondis) est **pris entier**,
convention de `lib/owner-rankings.ts` appliquée à une sélection éditoriale. Et la **coupe se fait sur
un écart franc**, Quimper 19,7290 % contre Saint-Pierre de La Réunion 19,5238 %, donc aucun palier
n'est coupé en son milieu. Huit communes éligibles sur les chiffres sont écartées comme banlieues de
métropoles déjà couvertes, au même titre que Villeurbanne aux batches 3 et 4 : Villeurbanne (29,2 %),
Ivry-sur-Seine (25,1 %), Levallois-Perret, Versailles, Courbevoie, Boulogne-Billancourt,
Issy-les-Moulineaux et Rueil-Malmaison. ⚠️ Rappel du batch 4, toujours valide : **le seuil de
population se prend sur `pop2022`, pas sur le `population` du seed**.

**C'est le premier lot de la série qui sort de métropole, et Saint-Denis de La Réunion inverse à peu
près tout ce que les trente-quatre guides précédents avaient posé.** Sa part de 60 ans et plus,
20,0 %, est la plus basse du lot pour une médiane nationale de 27,2 %, et c'est la seule ville des
sept qui croisse fortement (+5,6 % depuis 2016, +7,4 % depuis 2011). Elle porte le meilleur score de
vie quotidienne (6,8/10) et de nature (8,4/10) du lot, et les deux plus mauvais sur le coût (4,1/10)
et le transport (4,7/10). Trois faits structurent son guide et sont vérifiés en ligne : **il n'existe
aucun train à La Réunion**, l'ancien chemin de fer ayant fermé en **1976** et le tram-train ayant été
**abandonné en 2010** après le changement de majorité régionale, les **435 M€** de crédits d'État
étant redéployés vers la route du littoral au profit du programme de BHNS **Trans Éco Express** ;
le réseau urbain **Citalis** dessert Saint-Denis, Sainte-Marie et Sainte-Suzanne ; et **la saison
sèche correspond à l'hiver austral**, de mai à novembre, la saison cyclonique courant de novembre à
avril avec un pic en janvier-février-mars, ce que nos propres normales confirment (14 jours de pluie
en janvier, février et mars contre 6 en novembre). Le guide dit aussi que le poste de budget non
partageable n'y est pas la chambre mais **le billet d'avion et la location de voiture**, facturée au
véhicule.

⚠️ **Dix comparaisons inter-villes fausses, écrites au premier jet et corrigées avant commit — le
mode de défaillance des batches 3 et 4 s'est reproduit une troisième fois, et la parade prescrite l'a
de nouveau entièrement rattrapé.** Les scores *de la ville de chaque page* étaient tous justes ; ce
sont les rangs et les superlatifs **entre** villes qui dérapent, parce qu'on les écrit de mémoire, et
ni `tsc` ni `npm run integrity` ne peuvent les voir. Le remède reste le même et doit être appliqué
avant toute relecture : **sortir la matrice des axes du lot avec ses paliers d'ex æquo, et vérifier
chaque superlatif contre elle**. Les dix :
- **Mulhouse** annoncée « les loyers de référence les plus bas des sept » dans son intro, sa section
  budget **et sa `metaDesc`** : c'est **Bourges**, en dessous sur les deux références (410 € contre
  420 € en T1, 570 € contre 580 € en T2).
- **Valence** donnée « deuxième derrière Saint-Denis » sur la nature (6,3) : **Quimper la précède**
  à 6,4.
- **Valence** donnée « avant-dernière » en sécurité (5,2) : elle est la **quatrième plus basse**,
  Saint-Denis 4,9 étant en dessous.
- **Saint-Denis** donnée « quatrième valeur la plus basse » en sécurité (4,9) : elle est la
  **troisième**, derrière Avignon 4,3 et Mulhouse 4,5.
- **Avignon** donnée « troisième score de coût le plus défavorable » (5,7) : elle est la **deuxième**,
  derrière Saint-Denis seule.
- **Avignon** : son avance de culture chiffrée à « trois points et demi » sur le minimum du lot, alors
  que 9,0 − 6,2 = **2,8**. Même famille que l'écart de sept siècles et demi lu « deux siècles et demi »
  à Sens (batch 37) : les deux nombres étaient justes, c'est la **soustraction** qui dérapait.
- **Avignon** : novembre annoncé « plus ensoleillé que la moyenne de juillet en Bretagne », alors que
  Quimper est à **5,8 h** en juillet contre 5,0 h à Avignon en novembre. Comparaison retirée.
- **Quimper** donnée « avant-dernière » en transport (5,5), la phrase se contredisant elle-même en
  ajoutant « devant Bourges et Saint-Denis » : elle est la **troisième plus basse**.
- **Quimper** donnée « quatrième du lot » sur le coût (6,2) : elle est **cinquième**.
- **Bourges** présentée comme « la deuxième ville de la série sans TGV ni mode urbain sur rails, après
  Troyes » : elles sont **trois**, et **ce batch en apporte deux d'un coup** (Bourges et Saint-Denis).
  Le contrôle qui l'a tranché est un `npx tsx` de quinze lignes sur les 36 slugs de la série contre
  `lib/transit.ts` — il ne coûte rien et il faut le relancer à chaque batch.

⚠️ **Le croisement `neighborhoods` ↔ `transit` recommandé par le batch 4 a été relancé, et il est
propre cette fois** : aucun tramway ni métro fantôme neuf sur les villes dont `lib/transit.ts` porte
une fiche renseignée. Trois cas remontent, tous en Île-de-France et tous **exacts** après lecture
(Champerret à Levallois est bien desservi par le T3b sur le boulevard des Maréchaux, Nanterre et
Courbevoie décrivent la desserte de La Défense par la ligne 1, qui est une desserte accessible et non
une prétention communale). La variante brute du contrôle, sans le filtre « fiche transit renseignée »,
sort **101 cas dont la quasi-totalité sont des faux positifs** : une banlieue sans fiche transit
(`{}`) qui mentionne le tram de la métropole voisine dit vrai. **Garder le filtre.**

**Trois autres pièges de localisation et de calendrier écartés avant rédaction**, convention
« accessible depuis » tenue partout : ① la **gare de Valence TGV est sur la commune d'Alixan**, à une
dizaine de kilomètres du centre, reliée à Valence-Ville par un TER d'environ **7 minutes** et une
vingtaine de circulations par jour — un billet pris sans regarder la gare dépose le voyageur dans une
autre commune, et le guide le dit avant tout le reste ; à comparer avec **Avignon**, dont la gare TGV
est bien **sur la commune** (quartier de la Courtine) et reliée à Avignon-Centre par la **Virgule**,
raccordement ouvert le **15 décembre 2013**, **4 minutes**, environ **35 allers-retours par jour** —
deux gares excentrées, deux situations différentes, et c'est exactement le genre de nuance qu'une page
recopiée d'une autre écrase. ② Le **Train-Théâtre est à Portes-lès-Valence**, pas à Valence, et la
**Comédie itinérante** de la Comédie de Valence joue plus de **60** représentations par saison **hors
de la ville**, avec plus de **110** partenaires locaux : vérifier le lieu avant de prendre un billet.
③ Le **Séchoir est au Piton Saint-Leu** et **Kabardock au Port**, pas à Saint-Denis ; le **Téat Champ
Fleuri**, lui, est bien à Saint-Denis.

**Trois calendriers déjà passés à la publication, écrits comme tels plutôt qu'au présent**, précédent
du Festival de Loire au batch 4 : les **24 Heures du Mans** (94ᵉ édition les **13 et 14 juin 2026**,
suivante attendue en juin 2027), le **Festival d'Avignon** et le **Festival Off** (**4 au 25 juillet
2026**, 80ᵉ édition du IN et soixante ans du OFF, plus de **1 700** spectacles et plus de **130**
théâtres pour le OFF, suivante attendue en juillet 2027), le **Printemps de Bourges** (50ᵉ édition du
**14 au 19 avril 2026**, environ **130** artistes pour près de **300** concerts, suivante attendue en
avril 2027) et le **Festival de Cornouaille** de Quimper (103ᵉ édition du **23 au 26 juillet 2026**,
plus de **2 000** artistes et près de **150** rendez-vous). Dans les quatre cas le guide dit que c'est
la semaine où l'hébergement se tend et où une chambre non partagée coûte son maximum annuel, ce qui
est le sujet même de la série.

**Un superlatif conservé parce qu'il est vérifié, et sa formulation prudente.** La **maison de la
culture de Bourges** a ouvert en **octobre 1963** et a été inaugurée par **André Malraux le 18 avril
1964** ; celle du **Havre l'avait été avant elle, en 1961**, mais Bourges est **la première à avoir
fonctionné selon les principes posés par le ministre**. Écrire « la première maison de la culture de
France » aurait été faux, et c'est la formulation que tout le monde recopie. Autres faits vérifiés en
ligne avant rédaction : **Bourges capitale européenne de la culture 2028**, désignée le **13 décembre
2023** devant Clermont-Ferrand, Montpellier et Rouen, cinquième ville française après Paris, Avignon,
Lille et Marseille-Provence, aux côtés de České Budějovice et Skopje — le guide en tire un conseil de
calendrier long, **2026 et 2027 sont la bonne fenêtre**, 2028 étant l'année où l'hébergement se
tendra ; **Bourges n'a ni TGV ni rail urbain**, la liaison se fait en **Intercités depuis Paris-Bercy
via Étampes et Vierzon, environ 2 h 15, quatre allers-retours par jour** ; **tramway du Mans** T1 le
**19 novembre 2007** et T2 le **30 août 2014**, près de **19 km** et **33 stations**, exploité par la
**Setram** ; **Superforma**, seule SMAC de la Sarthe, labellisée en **2015**, et **Les Quinconces et
L'Espal** labellisés scène nationale en **décembre 2018** ; la **Nuit des Chimères** habille la Cité
Plantagenêt chaque soir d'été jusqu'à la fin septembre, **gratuitement et sans réservation**, ce qui
en fait le meilleur argument du lot pour une soirée en solitaire ; **tramway de Mulhouse** lignes 1 et
2 le **20 mai 2006**, ligne 3 le **12 décembre 2010** en même temps que le **tram-train de la vallée
de la Thur**, premier tram-train réellement interconnecté de France, **16,2 km** et **29 stations** ;
le **Noumatrouff**, SMAC née en **1992** dans une friche du quartier de la Mertzau, **1 500 m²**, deux
salles de **600** et **300** places, **80 concerts** par an, près de **25 000** spectateurs ; la
**Cité de l'Automobile**, plus grande collection automobile exposée au monde, **560+** véhicules dont
**130 Bugatti**, dans une ancienne filature rachetée par les frères Schlumpf en **1957** et ouverte au
public en **1982** ; **centre historique d'Avignon inscrit à l'UNESCO en 1995**, **4,3 km** de
remparts des XIVᵉ et XVᵉ siècles, **université d'Avignon environ 8 000 inscrits** (ordre de grandeur
sans rapport avec les villes étudiantes de la série, ce qui fait de la vitalité avignonnaise un
mécanisme d'économie culturelle et non de population étudiante, et le guide le dit) ; **tramway
d'Avignon** mis en service le **19 octobre 2019**, une ligne, **5,2 km**, réseau **Orizo** ; **CDN de
l'océan Indien** labellisé le **30 juillet 2018**, Théâtre du Grand Marché **300 places** plus une
salle cabaret de **200**, seconde salle **FabriK** à Sainte-Clotilde ; **Quimper–Paris-Montparnasse en
3 h 30**, **six allers-retours** quotidiens depuis l'ouverture de la **LGV Bretagne-Pays de la Loire
en 2017** ; **Le Novomax** de Quimper, cinq studios, salle modulable **Le Club** de **100 à 390**
places, bureaux de **Polarité[s]**.

**Chiffres et vérifications.** Les **107** figures en `/10` des sept guides tracent **toutes** vers un
score **rendu** (`CITIES_SEED` et `data/neighborhoods.ts` lus via un `npx tsx`, jamais par grep du
seed) d'une ville que le guide nomme ; les parts d'âge et les populations viennent de
`lib/city-population.ts` (Insee 2022), **pas des `population` approximatives du seed** ; les loyers
viennent de `data/housing.ts` et les températures, heures de soleil et jours de pluie de
`lib/vacation-seasons.ts`. **Aucun tarif d'hébergement n'est imprimé**, comme dans toute la série : on
transmet le réflexe, pas le prix. `metaTitle` 57-60 caractères, `metaDesc` 143-153, 6 sections et
1 287-1 408 mots par guide, densité d'accents 0,132-0,159 (**par mot** ; seuil de détection
ascii-strip 0,09), **zéro em-dash dans le corps**. `relatedGuides` câblés sur des slugs vérifiés
existants, `relatedCities` sur la ville cible. Garde-fou éditorial tenu : aucune promesse de
rencontre, aucun registre « site de rencontres », aucun cliché sur la solitude, écriture inclusive
légère (`seul·e`), et la distinction avec le profil `solo` portée par la section « sortir un mardi
soir » de chaque guide. Deux villes portent un score de sécurité bas (Avignon 4,3, Mulhouse 4,5) : le
chiffre est dit, sans verdict sur les habitants, comme Grenoble et Lille l'ont été aux batches 3 et 4.
...
prescrit passe en entier : `npx tsc --noEmit` **propre**, `npm run integrity` (guides FR 1073 →
1080), `search-index` + `search-index:check`, `sitemap:check`, `npm run parity` (0 route FR sans
jumelle), `npm run hreflang:check`, plus les contrôles maison du run (matrice des superlatifs,
traçage des figures `/10`, résolution `guideCityPhoto`, croisement `neighborhoods` ↔ `transit`,
vérification d'encodage). Note d'environnement reconfirmée : le conteneur de routine démarre **sans
`node_modules`** — `npm install` d'abord.

**Écart FR→EN après ce batch : 7 villes** (le-mans, valence, saint-denis-reunion, mulhouse, avignon,
bourges, quimper) — au-dessus du seuil de ~6, donc **le prochain run doit être un batch EN** de la
série `solo-travel-in-[city]-2026`. Rappel de la règle du batch 33 : côté EN le slug se dérive du
**slug de seed tel quel**, donc **`solo-travel-in-saint-denis-reunion-2026`** garde son suffixe (le
lookup se fait sur le slug de seed) et **`solo-travel-in-le-mans-2026`** garde son article. Quatre
points de vigilance pour ces jumelles. ① **Saint-Denis de La Réunion doit être désambiguïsé dès la
première ligne** face à Saint-Denis (93), sur le modèle déjà en place dans la série tourisme, et
l'inversion des saisons doit être posée avant toute phrase sur le climat : un lecteur de l'hémisphère
nord lira « hiver » comme une mauvaise saison. ② **Le Mans est de l'histoire du sport automobile
britannique et américaine autant que française** (Bentley, Jaguar, Ford face à Ferrari, Steve
McQueen), donc la jumelle anglaise a de la matière propre là où le guide FR reste sobre et se
contente de dire d'éviter la date : même arbitrage qu'avec Jubilee à Dieppe (batch 37) et Cambrai
(batch 39). ③ **Mulhouse est à portée de Bâle**, donc la jumelle doit poser que la Suisse est dans
Schengen mais **hors Union européenne et hors union douanière** — pièce d'identité obligatoire et
franchises applicables, le piège étant de déduire « pas de contrôle » de « pas de règles » ; c'est le
cadrage déjà retenu pour Thonon au batch EN 41. ④ **Le Festival d'Avignon se joue en anglais dans les
sources anglophones sous son nom français**, et le OFF est le plus gros marché de spectacle vivant
d'Europe : la jumelle a de quoi être plus longue que la française sur ce seul point.
Pour le batch FR **suivant**, la règle recalculée donnera, dans l'ordre et hors banlieues de
métropoles : **Saint-Pierre de La Réunion 19,5 % (199ᵉ)**, **Colmar 19,4 % (209ᵉ)** et **Annecy 19,4 %
(212ᵉ)** — ces deux dernières formant un **palier d'ex æquo à prendre entier ou pas du tout** — puis
**Niort 19,0 % (236ᵉ)**, **Nice 18,4 % (268ᵉ)**, **Montauban 17,8 % (301ᵉ)**, **Fort-de-France 15,8 %
(420ᵉ)**, **Antibes 14,5 % (465ᵉ)**, **Cannes 14,4 % (468ᵉ)** et **Ajaccio 13,7 % (483ᵉ)**. Les
recalculer plutôt que les recopier. Deux remarques. Le vivier bascule vers **le littoral et les
stations**, c'est-à-dire exactement le profil que l'anti-station-fantôme est fait pour écarter :
Cannes, Antibes et Ajaccio sont sous la médiane nationale de 18,4 % et devront être traitées comme
telles, ou écartées, plutôt que vendues. Et **Fort-de-France ouvrirait la série aux Antilles** après
La Réunion, avec le même travail d'inversion de calendrier à faire, la saison sèche antillaise
(le carême) courant de décembre à avril, donc à l'inverse de celle de La Réunion.
...
### F58 — série `parent-solo-a-[ville]-2026`, batch 9 (2026-09-06) — les paliers 6,7 et 6,6 sont refermés

**+10 guides : Alençon, Brive-la-Gaillarde, Étampes, Vichy, Beaune, Dieppe, Dreux, Laval, Roanne,
Saint-Quentin.** Compteurs mesurés des deux côtés **avant** le run, comme les batches 4 à 8
l'exigent : `grep -c 'slug: "parent-solo-a'` = 75 FR et `grep -c 'slug: "single-parent-in-'` = 75 EN,
mêmes villes de part et d'autre, la main revenait donc au FR. Après ce run : **85 FR / 75 EN**.
`GUIDES` 1 080 → 1 090. Les 10 guides sont vérifiés **retrouvés par le lookup de
`app/villes/[slug]/parent-solo/page.tsx`** (`parent-solo-a-${slug}-2026` sur le **slug de seed tel
quel**) **et pourvus de leur photo d'en-tête** (`guideCityPhoto`). Aucun slug hors gabarit : les dix
villes prennent « à » sans contraction.

⚠️ **Le batch prend 10 villes et non 9, et la raison est une règle et pas une commodité : une égalité
ne se coupe jamais en son milieu.** La règle du rang posée au batch 7 désignait les communes non
couvertes les mieux classées ; le vivier recalculé (jamais recopié) donne 4 communes à 6,7 (Alençon,
Brive-la-Gaillarde, Étampes, Vichy) et 6 à 6,6 (Beaune, Dieppe, Dreux, Laval, Roanne, Saint-Quentin).
S'arrêter à 9 aurait coupé le palier 6,6 en son milieu et fabriqué une hiérarchie entre des villes que
le score ne départage pas, exactement ce que `lib/owner-rankings.ts` interdit ailleurs. Prendre les 10
referme les deux paliers : **toute commune notée 6,6 ou plus au composite a désormais son guide**, et
c'est une frontière vérifiable, contrairement à « le top 29 ». Le palier suivant, 6,5, compte
**10 communes non couvertes** (Abbeville, Castres, Charleville-Mézières, Châtellerault,
Issy-les-Moulineaux, Levallois-Perret, Montbéliard, Saint-Dizier, Saumur, Villeneuve-sur-Lot) : c'est
le vivier du prochain batch FR, **à recalculer et non à recopier**. Exclusions maintenues : le cluster
francilien dense et le cluster nordiste restent hors série au titre de la règle granulaire des
batches 4 à 6 ; aucune des dix villes de ce lot n'en relevait, la plus basse sur les écoles étant
Saint-Quentin à 5,5/10 et sur la sécurité Saint-Quentin à 4,7/10.

⚠️ **Le fait le plus utile du run est la correction d'une erreur publiée dans 11 guides de la série,
et elle vient d'un accesseur mal employé.** Onze guides affirmaient que `data/neighborhoods.ts` **ne
documente aucun quartier** pour leur ville : `parent-solo-a-valence`, `-colmar`, `-calais`,
`-le-tampon`, `-ajaccio`, `-la-roche-sur-yon`, `-lannion`, `-challans`, `-vienne`,
`-villefranche-sur-saone` et `-chaumont`. C'est faux, et le compte-rendu du batch 8 généralisait
l'erreur à ses neuf villes. Le fichier documente **trois quartiers pour chacune des 540 communes**
(deux pour Le Tampon et Saint-Paul), Lyon et Sens compris. La cause est mécanique et vaut d'être
retenue : **`NEIGHBORHOODS` est un tableau, pas un dictionnaire**, donc `NEIGHBORHOODS[slug]` rend
`undefined` pour toute ville et un script de contrôle écrit ainsi conclut « aucun quartier » partout,
sans lever d'erreur. L'accesseur est `getNeighborhoods(slug)`. Le tell qui a démasqué le bug est
d'avoir passé **Lyon** dans le même contrôle : une ville dont on sait qu'elle a des quartiers
documentés. **Règle générale : un contrôle qui rend « absent » pour tout le corpus se teste d'abord
sur un cas dont on connaît la réponse.** Les 11 guides sont corrigés vers l'affirmation vraie, qui
préserve la position éditoriale sans reposer sur une prémisse fausse : le fichier documente trois
quartiers, sur une échelle qui leur est propre et qui ne se compare pas au score communal, et **trois
observations ne décrivent pas une commune**, ce qui est la raison affichée du refus de publier un
verdict de sécurité par secteur. Les 10 guides neufs portent la même formulation. La décision
d'écarter la série `quartiers-a-eviter` est inchangée et repose désormais sur le bon motif.

**Cinq superlatifs faux corrigés AVANT insertion**, par requête et non par relecture, le mode de
défaillance des batches 5 à 8 tenant toujours : ① le guide Beaune donnait sa baisse de population
« la plus forte des dix villes de ce lot » alors que **Dieppe perd 8,2 % contre 7,5 %** ; ce qui est
vrai de Beaune est le **rythme récent** (6,5 % sur six ans contre 3,4 % à Dieppe), et la phrase dit
maintenant les deux. ② Le guide Roanne annonçait « le deuxième prix au mètre carré le plus bas du lot
après Laval » : Roanne est à 1 500 € **à égalité avec Saint-Quentin**, et c'est **Alençon à 1 400 €**
qui mène ; palier nommé. ③ Le guide Dreux créditait Tours de **7,4/10 d'écoles**, valeur de Besançon,
Caen et Dijon ; Tours est à **7,9**, corrigé aux deux endroits. ④ Le guide Alençon donnait le seuil de
Caen à 2 900 € (c'est celui de Dijon et de Rouen) alors qu'il est à **2 800 €**, et « mille euros
d'écart » devient neuf cents. ⑤ Les seuils de Vichy et de Laval étaient présentés comme deuxième et
troisième du lot sans voir que **Vichy et Saint-Quentin sont à égalité à 1 950 €** ; les deux guides
nomment le palier. Un sixième point a été corrigé pour imprécision plutôt que pour fausseté : le guide
Étampes écrivait « l'une des vingt meilleures du top 40 », remplacé par la mesure exacte, **la moitié
des quarante premières communes atteint ou dépasse 7,7/10 d'écoles**. Et le guide Alençon comparait
ses écoles à celles de « Caen comme Rouen » : **Rouen est à 6,6, c'est-à-dire à égalité**, seule Caen
fait mieux.

**Vérification des pivots automatisée, reconduite des batches 4 à 8** : un script rejoue **après
insertion** fit, rang, T1/T2/T3, prix au m², prix des 65 m², revenu minimum, ratio loyer ÷ écoles,
populations Insee 2011/2016/2022 et les quatre axes de chacune des dix villes, et échoue si l'une des
valeurs n'est pas retrouvée dans le texte. Les 10 passent. Tous les chiffres sont lus **à travers les
modules** (`npx tsx` sur `@/data/cities-seed`, `@/data/housing`, `@/lib/city-population`,
`@/lib/parent-solo`, `@/data/neighborhoods`), jamais par grep du seed, et le rang est rejoué avec le
départage réel de `app/parent-solo/page.tsx` (`name.localeCompare(…, "fr")` à égalité de score).
`metaTitle` 50-58 caractères, `metaDesc` 141-151, 6 sections par guide, 1 652-1 831 mots, densité
d'accents 0,129-0,160 **par mot** (seuil ascii-strip 0,09), **0 em-dash hors `metaTitle`** (un par
guide, séparateur structurel de la série). `npm run search-index` relancé
(`data/search-index.json` 1 090 guides, 263 tags), `npm run sitemap:check` (FR 29 185 URL,
EN 28 746), `npm run parity` (0 route FR sans jumelle) et `npm run hreflang:check` verts. Un `m2`
ascii avait été écrit dans un tag Roanne : corrigé en `m²`, la convention du fichier (`prix m²`).

**Le fil éditorial du lot est démographique, et il est mesuré, pas supposé.** C'est le premier batch
de la série où **la majorité des villes perd des habitants : 7 sur 10** sur 2011-2022, quand
**100 des 361 communes classées** dont la population est mesurée sont dans ce cas. La mécanique est
identifiée par une seconde mesure : parmi les **30 communes au meilleur rapport loyer ÷ point
d'écoles, 20 perdent des habitants**. Un loyer bas rapporté à l'offre scolaire est donc, pour une
bonne part, le prix d'un marché détendu par les départs, et les deux faits n'en font qu'un vu de deux
côtés. C'est la généralisation, à l'échelle du classement, de ce que le guide Chaumont posait pour la
Haute-Marne au batch 8. Chaque guide en tire la conséquence pratique, qui joue dans les deux sens :
un marché détendu fait accepter un dossier avec un seul revenu, une commune qui se vide resserre sa
carte scolaire, et **aucun des quatre axes ne mesure cela**. Trajectoires réelles lues dans
`data/city-population.json` : **Étampes +10,8 %** (24 013 → 26 601, la seule en croissance franche,
+1,43 %/an), Vichy +2,8 % et Dreux +2,2 %, puis Roanne −2,2 %, Alençon −2,4 %, Laval −2,7 %,
Brive −3,1 %, Saint-Quentin −5,8 %, **Beaune −7,5 %** et **Dieppe −8,2 %**. Trois formes distinctes à
ne pas confondre : **Vichy et Roanne dessinent un V** (baisse jusqu'en 2016 puis reprise nette),
**Brive et Laval ont vu leur décroissance s'arrêter** (Laval perd 18 habitants en six ans, une
stabilité stricte), et **Beaune accélère sa chute** (−1,12 %/an). ⚠️ Le seed conserve ses `population`
approximatives, qui servent au seuil des 20 000 et aux tris : **les deux nombres coexistent
volontairement**, ne pas les aligner, et dire lequel est cité. Les guides Étampes, Vichy, Dreux et
Laval le disent explicitement, l'écart y étant visible (Étampes en compte 2 601 de plus au
recensement que dans le seed).

**Six cas valent d'être gardés en tête.** ① **Alençon tient le meilleur rapport des 363 à 94 €** de
loyer par point d'offre scolaire, devant le trio Auch, Vichy et Cambrai à 97 € et le duo Laval,
Auxerre à 100 €. Elle l'obtient **par le seul loyer** (écoles à 6,6/10, la note la plus courante du
classement), donc du type Auch et non du type Dole, et son guide le dit pour ne pas laisser croire à
une offre scolaire remarquable. ② **Brive-la-Gaillarde a le profil le plus plat des 363** : 0,4 point
entre son meilleur et son plus faible axe, devant Colmar et Vernon à 0,5. Sens, au batch 8, vendait sa
régularité à 1,2 point ; Brive fait trois fois mieux, et **Dieppe la suit à 0,8**, deuxième du top 40.
Deux profils plats dans un même lot, ce que la série n'avait jamais eu. ③ **Étampes est la deuxième
des 115 communes franciliennes classées** derrière Fontainebleau, et l'écart départemental est le
fait : la deuxième de l'Essonne est **Massy à 6,0**, sept dixièmes plus bas, parce que les communes
essonniennes les mieux desservies sont les plus chères (Massy 9,0/10 de transports pour 4,6/10 de coût
et un T3 à 1 650 €). ④ **Beaune est posée exactement sur 5,0 de coût de la vie**, la frontière de la
règle des 35 % de `lib/parent-solo.ts` : la condition étant un strict inférieur, elle reste du côté du
calcul strict, et elle est **la mieux classée des 22 communes à ce score**. Dans le top 40, seules
Anglet, Fontainebleau et Lyon passent réellement dessous. ⑤ **Laval est le second guide de la série
dont le verdict renvoie le lecteur ailleurs**, après Soissons au batch 8 : elle est 6e de sa région
derrière cinq villes **déjà couvertes par la série**, et La Roche-sur-Yon, 1re des 363, offre 1,1 point
d'écoles et 1,2 point de sécurité de plus pour 140 € de T3 mensuel. Son guide le dit plutôt que de
vendre son rang. ⑥ **Saint-Quentin referme le trio de l'Aisne**, dont les trois communes classées
partagent **exactement le même score de coût de la vie, 8,3/10**, que seules 5 communes des 363
dépassent : ce qui les départage est ailleurs, Saint-Quentin prenant les transports (7,7 contre 6,4 et
5,6) et le loyer (650 € contre 740 € et 930 €) et cédant les écoles (5,5 contre 6,6) et la sécurité
(4,7 contre 6,5), d'où vingt-sept rangs d'écart avec Laon.
...
redit ; ② la note de sécurité est **communale** et ne dit rien d'un quartier, d'une rue ni des
personnes qui y vivent, ce que les guides Étampes, Dreux, Roanne et surtout **Saint-Quentin** (4,7/10,
l'axe le plus faible du lot) écrivent explicitement ; aucun loyer de quartier n'est cité ; ③ les
guides **nomment les villes qui font mieux qu'eux**, y compris hors série (Alençon face à Dieppe et à
Brive sur le rapport ; La Roche-sur-Yon face à Laval ; Dole face à Beaune ; Laon face à
Saint-Quentin ; Fontainebleau face à Étampes ; Vichy face à Roanne) ; ④ trois contraintes
hors-composite sont posées parce qu'elles décident d'un budget et qu'aucun axe ne les voit : la
**saisonnalité de l'emploi et la bascule d'une part du parc vers la résidence secondaire** à Dieppe
(même traitement qu'aux Sables-d'Olonne au batch 8), la **concurrence des navetteurs franciliens** sur
le marché locatif drouais (même mécanique que Sens), et le fait que le score de coût de la vie
**mesure des prix et non des salaires**, dit dans le guide Roanne ; ⑤ **41,5 % des habitants de Vichy
ont 60 ans ou plus**, cinquième part la plus forte des 363 à égalité avec Six-Fours-les-Plages, contre
**18,6 % à Étampes**, la plus faible du lot : deux communes au même score de 6,7/10 et deux
environnements sans rapport, ce qu'un score de synthèse ne peut pas porter. C'est écrit comme une
mesure, sans jugement sur la commune ni sur ses habitants.

**Prochain run parent-solo : le miroir EN** (`single-parent-in-[city]-2026`), l'écart étant de 10.
Vérifier le compteur des deux côtés avant de suivre cette phrase, consigne des batches 4 à 8. Nommage :
**le slug EN se dérive du slug de seed tel quel** (règle du batch 33 tourisme), donc
`single-parent-in-brive-la-gaillarde-2026` et `single-parent-in-saint-quentin-2026` gardent leur forme
complète. Quatre points de vigilance pour ces jumelles : **`single-parent-in-laval-2026` est un slug
ambigu en anglais**, Laval étant d'abord une ville du Québec pour un lecteur nord-américain, et l'intro
doit poser la Mayenne dès la première ligne, comme le batch 39 l'a fait pour Vernon ;
**`single-parent-in-saint-quentin-2026`** vise l'Aisne et non le mont Saint-Quentin de la Somme, piège
déjà signalé au batch 34 tourisme ; **`single-parent-in-beaune-2026` a de la matière propre à l'angle
anglophone**, le vignoble bourguignon étant un repère connu, mais la jumelle doit garder l'arbitrage du
FR (un foncier valorisé sur une demande non locale contre des salaires locaux) et ne pas glisser vers
le guide touristique ; et **`single-parent-in-etampes-2026` doit reprendre l'avertissement de seuil**,
un lecteur étranger qui lit « 2nd in the Paris region » sans voir les 3 400 € de revenu minimum se
fera une idée fausse de l'accessibilité.

### F58 — série `parent-solo-a-[ville]-2026`, batch 8 (2026-09-04) — le top 20 du classement est refermé

**+9 guides : Fontainebleau, Soissons, Vienne (38), Villefranche-sur-Saône, Auch, Chaumont,
Compiègne, Les Sables-d'Olonne, Sens.** Compteurs mesurés des deux côtés **avant** le run, comme les
batches 4 à 7 l'exigent : `grep -c 'slug: "parent-solo-a'` = 66 FR et
`grep -c 'slug: "single-parent-in-'` = 66 EN, mêmes villes de part et d'autre — écart nul, la main
revenait donc au FR. Après ce run : **75 FR / 66 EN**. `GUIDES` 1 057 → 1 066. Les 9 guides sont
vérifiés **retrouvés par le lookup de `app/villes/[slug]/parent-solo/page.tsx`**
(`parent-solo-a-${slug}-2026` sur le **slug de seed tel quel**) **et pourvus de leur photo
d'en-tête** (`guideCityPhoto`). Un slug hors gabarit, conforme à la convention déjà en place
(`parent-solo-a-le-havre-2026`, `parent-solo-a-le-mans-2026`, `retraite-a-les-sables-d-olonne-2026`) :
**`parent-solo-a-les-sables-d-olonne-2026`** garde l'article, parce que le lookup se fait sur le slug
de seed ; le titre et la copie écrivent « aux Sables-d'Olonne ». Ne pas le « corriger » en `aux-`.

**La règle du rang, posée au batch 7, a été appliquée telle quelle et le vivier a été recalculé, pas
recopié.** Le calcul de ce run redonne exactement les neuf communes non couvertes les mieux classées,
c'est-à-dire les rangs 12 à 20 : **le top 20 du classement publié est désormais couvert en entier**
(les rangs 1 à 11 l'étaient depuis le batch 7). C'est le premier batch de la série où la liste
candidate n'a demandé aucun arbitrage. `npm run search-index` relancé (`data/search-index.json`
1 066 guides, 254 → **257 tags**, trois tags franchissant le seuil de 3 guides), d'où le passage de
`npm run sitemap:check` (FR 29 155 URL, EN 28 731, chaque URL déclarée a une page et réciproquement).

⚠️ **Le fait le plus utile du run est une mesure sur nos propres données : le seuil d'éligibilité du
classement s'applique à la population *approximative du seed*, et cinq communes y figurent que le
recensement met sous la barre.** Mesuré ce run sur les 540 villes : **Fontainebleau (seed 25 000,
recensement 2022 15 787), Saint-Dié-des-Vosges (20 000 / 19 324), Sélestat (20 000 / 19 523), Pertuis
(22 000 / 19 764), Cahors (20 000 / 19 902)** ; et cinq communes sont dans le cas inverse, au-dessus
de 20 000 au recensement et absentes du classement (**Givors, Lingolsheim, Denain, Cugnaux,
Mont-Saint-Aignan**). Ce sont les mêmes dix que l'édition de novembre du palmarès mensuel avait
relevées, la nouveauté est de mesurer l'**ampleur** de l'écart : quatre des cinq entrants sont à moins
de 700 habitants du seuil, **Fontainebleau en est à 4 213**, et son écart seed/recensement de 9 213
habitants est le plus gros du corpus classé. Le guide Fontainebleau le dit **dans son intro et dans
une section entière**, plutôt que de publier un rang « 1re d'Île-de-France » sans dire sur quoi il
repose. ⚠️ **Ne pas « corriger » le plancher de `app/parent-solo/page.tsx` pour lire
`lib/city-population` depuis une routine** : cela redistribue les 363 rangs et périme les rangs cités
dans les 75 guides de la série. C'est un changement produit, à faire avec un build local et une passe
de réécriture des guides, pas un correctif d'affichage.

**Cinq superlatifs faux corrigés AVANT insertion**, par requête et non par relecture — le mode de
défaillance des batches 5 à 7 tient toujours : **un superlatif est une requête, pas une intuition.**
① Le guide Villefranche annonçait sa ville « seule des dix communes à 8,7/10 d'écoles dont le T3 reste
sous 1 100 € » : **Strasbourg est à 1 080 €** ; corrigé en « le T3 le plus bas des dix, devant
Strasbourg », et le rapport de 123 € par point est bien le meilleur des dix (contrôlé). ② Le guide
Vienne donnait « quatre » communes du top 20 au-dessus de 7,5 sur les transports **et** les écoles ;
elles sont **cinq** (Rennes, Strasbourg, Anglet, Vienne, Compiègne). ③ Le guide Sens écrivait que Dole
« fait mieux sur tout ce qui se mesure » : Dole est **à égalité** sur le coût de la vie (7,6) et les
transports (6,4), meilleure sur les deux autres. ④ Le guide Compiègne chiffrait son loyer « supérieur
de 25 à 30 % à celui de ses voisines de l'Aisne » : l'écart réel est de **+5 % sur Soissons et +32 %
sur Laon** ; remplacé par les euros. ⑤ Le guide Chaumont donnait la sécurité de Saint-Dié-des-Vosges
« à un dixième près » : les deux sont **à 7,5 exactement**. Un sixième point, sur Auch, a été corrigé
pour cause d'imprécision plutôt que de fausseté : « la vingt et unième desserte la plus faible » est
un rang **à l'intérieur d'une égalité à dix communes** (Auch, Vichy, Alençon, Oyonnax,
Villeneuve-sur-Lot, Sarreguemines, Cavaillon, Pertuis, Allauch, Élancourt sont toutes à 4,2), donc
écrit comme un palier : « vingt communes seulement font moins bien, dont quatorze ultramarines ou
corses », conformément à la convention d'ex æquo de `lib/owner-rankings.ts`.

⚠️ **`npm run integrity` a levé un faux positif intéressant, et le remède est la prose, pas le
silence.** Le guide Auch citait « 5,5/10 de transports » **à propos de Castres** — valeur rendue
exacte — mais le garde de citation ne regarde que les **90 caractères qui précèdent** le chiffre pour
identifier la ville concernée : « Castres » était plus loin, la ville retenue était donc celle du
guide, et 5,5 se trouve être le littéral **brut** du seed pour Auch (rendu 4,2). La phrase a été
réécrite pour que « Castres » se retrouve à cinquante caractères du chiffre — ce qui la rend aussi
plus claire pour un lecteur humain, qui a le même problème d'attribution que le garde. **Règle
générale : quand ce garde parle sur une comparaison, rapprocher le nom de la ville du chiffre plutôt
que de contourner le contrôle.**

**Vérification des pivots automatisée, reconduite des batches 4 à 7** : un script rejoue **après
insertion** fit, rang, T1/T2/T3, prix au m², prix des 65 m², revenu minimum, ratio loyer ÷ écoles,
populations Insee 2011/2016/2022 et les quatre axes de chacune des neuf villes, et échoue si l'une des
valeurs n'est pas retrouvée dans le texte du guide. Il a rattrapé trois manques au premier passage
(Soissons sans son T1, Vienne sans son T2, Les Sables-d'Olonne sans ni l'un ni l'autre) ; les 9 passent
après correction. Tous les chiffres sont lus **à travers les modules** (`npx tsx` sur
`@/data/cities-seed`, `@/data/housing`, `@/lib/city-population`, `@/lib/parent-solo`), jamais par grep
du seed, et le rang est rejoué avec le départage réel de `app/parent-solo/page.tsx`
(`name.localeCompare(…, "fr")` à égalité de score). `metaTitle` 46-54 caractères, `metaDesc` 140-159,
6 sections par guide, densité d'accents 0,146-0,180 (**par mot, pas par lettre** — seuil ascii-strip
0,09), **0 em-dash sur les neuf guides**.

**Cinq cas valent d'être gardés en tête.** ① **Auch prend à Dole le meilleur rapport de la série** :
97 € de loyer par point d'écoles contre 101 €, mais **par l'autre mécanisme** — Dole obtient son
rapport des deux côtés (loyer bas *et* écoles à 7,7/10 supérieures à celles de ses deux métropoles
régionales), Auch par le seul loyer (640 €, écoles à 6,6/10, c'est-à-dire la moyenne). Le guide le dit
explicitement, et nomme les trois communes non couvertes qui font aussi bien ou mieux : **Alençon à
94 €, Vichy et Cambrai à 97 € comme Auch**. ② **Soissons est le seul guide de la série dont le verdict
renvoie le lecteur ailleurs.** Elle est 13e, mais **Laon marque exactement les mêmes notes de coût
(8,3), d'écoles (6,6) et de sécurité (6,5)**, fait mieux sur les transports (6,4 contre 5,6) et coûte
**190 € de T3 de moins** ; son rapport de 141 € par point est le **moins favorable des vingt premières
communes**. Le guide le dit dans son verdict plutôt que de vendre son rang. ③ **Chaumont tient le
mètre carré le moins cher des 363 (950 €, soit 61 750 € les 65 m²)**, devant Saint-Dizier à 1 000 € —
et les deux sont en **Haute-Marne**, qui perd 5,7 % et 8,1 % d'habitants en onze ans : le prix bas et
le vide sont le même fait vu de deux côtés, le guide le pose ainsi. Sa note d'écoles de 5,5/10 est la
plus basse du lot, donc son rapport de 115 € est du **type Auch**, pas du type Dole. ④ **Les
Sables-d'Olonne sont le cas où le classement ne voit pas ce qu'on paie** : mêmes écoles (7,7) et même
sécurité (8,0) qu'à La Roche-sur-Yon, 1re des 363, pour **800 € de revenu minimum mensuel de plus,
250 € de T3 de plus et 91 000 € de plus sur 65 m²**. Le guide chiffre le prix du littoral sans juger
qu'il ne vaut pas la peine. ⑤ **Sens est le contre-exemple utile de Fontainebleau** : 1,2 point
d'amplitude entre son meilleur et son plus faible axe, **deuxième profil le plus régulier du top 20**
derrière La Roche-sur-Yon (1,0) — contre 4,1 points à Fontainebleau — pour un seuil de 2 800 € contre
3 450 €. C'est exactement ce que le composite récompense, et les deux guides se citent.
...
guides nomment les villes qui font mieux qu'eux, y compris hors série (Alençon, Vichy et Cambrai face
à Auch ; Laon face à Soissons ; Auxerre face à Sens ; Saint-Dié-des-Vosges face à Chaumont ; La
Roche-sur-Yon face aux Sables-d'Olonne) ; ④ deux contraintes hors-composite sont posées parce qu'elles
décident d'une semaine et qu'aucun axe ne les voit : la **part de résidences secondaires et de
locations saisonnières** plus la **saisonnalité de l'emploi** aux Sables-d'Olonne (même traitement
qu'Anglet au batch 7), et la **concurrence des navetteurs franciliens** sur le marché locatif sénonais,
qui explique un T3 à 920 € pour une note de coût de la vie de 7,6/10 ; ⑤ la **règle des 35 %** de
`lib/parent-solo.ts` ne s'applique qu'aux villes dont le score de coût tombe sous 5,0 : **Fontainebleau
est, avec Anglet, l'une des deux seules communes du top 20 dans ce cas**, et son guide dit que son
seuil de 3 450 € est le **calcul indulgent** (3 650 € avec la règle stricte). Les sept autres guides du
lot disent l'inverse pour la même raison.

**Trajectoire démographique réelle**, lue dans `data/city-population.json` via `lib/city-population.ts`
et non dans les `population` approximatives du seed : Les Sables-d'Olonne **+17,5 %** (41 478 → 48 740,
**et ce n'est pas la fusion de 2019** — le fichier Insee rétropole sur la géographie communale
d'aujourd'hui, vérifié sur cette commune même à l'édition de novembre du palmarès), Vienne +9,6 %,
Sens +8,5 %, Fontainebleau +7,3 %, Auch +4,4 % **après un creux**, Compiègne +3,3 % **en
ralentissement**, Villefranche-sur-Saône +1,6 % sur onze ans mais **−2,8 % depuis 2016** (courbe en V
inversé, l'exact contraire de Dole), Soissons **+0,4 %**, la trajectoire la plus plate du lot, et
Chaumont **−5,7 %, avec un recul qui s'accélère** (−0,3 %/an puis −0,7 %/an). ⚠️ Le seed conserve ses
`population` approximatives, qui servent au seuil des 20 000 et aux tris : **les deux nombres
coexistent volontairement**, ne pas les aligner, et dire lequel est cité. Les guides Vienne, Compiègne,
Sens et Fontainebleau le disent explicitement, l'écart y étant visible.

**Prochain run parent-solo : le miroir EN** (`single-parent-in-[city]-2026`), l'écart étant de 9.
Vérifier le compteur des deux côtés avant de suivre cette phrase — consigne des batches 4 à 7, elle
vaut pour celle-ci. Nommage : **le slug EN se dérive du slug de seed tel quel** (règle du batch 33
tourisme), donc `single-parent-in-les-sables-d-olonne-2026` et
`single-parent-in-villefranche-sur-saone-2026` gardent leur forme complète ; la page ville résout son
guide par `getEnGuide('single-parent-in-' + slug + '-2026')` et une version « propre » du slug rendrait
le guide invisible. Trois points de vigilance pour ces jumelles : **`single-parent-in-vienne-2026` est
un slug ambigu en anglais** — *Vienna* est la capitale autrichienne, et les sources anglophones la
ramènent systématiquement ; l'intro doit poser Vienne (Isère) dès la première ligne, comme le batch 37
tourisme l'a fait pour Orange et le batch 39 pour Vernon (le guide FR le fait déjà, la contrainte est
plus forte côté EN). **`single-parent-in-sens-2026`** se lit comme le mot anglais *sens*/*sense* et
demande le même traitement. Et **`single-parent-in-fontainebleau-2026` doit reprendre l'avertissement
de population** : un lecteur étranger qui lit « 1st in the Paris region » sans savoir que la commune
compte 15 787 habitants se fera une idée fausse de la taille du bassin d'emploi.

Pour le batch FR **suivant**, le top 20 étant refermé, la règle du rang porte sur les rangs 21 et
au-delà. Vivier **à recalculer, jamais à recopier** : au moment de ce run, les communes non couvertes
les mieux classées sont **Alençon** (6,7, et le meilleur rapport des 363 à 94 €), **Étampes** (6,7),
**Brive-la-Gaillarde** (6,7), **Vichy** (6,7), **Beaune** (6,6), **Dieppe** (6,6), **Dreux** (6,6),
**Laval** (6,6, deuxième meilleur rapport à 100 €), **Roanne** (6,6) et **Saint-Quentin** (6,6).
Exclusions maintenues : le cluster francilien dense et le cluster nordiste (Roubaix, Tourcoing) restent
hors série au titre de la règle granulaire des batches 4 à 6 — on n'écrit pas de guide sur une commune
dont le composite est largement un proxy de la situation socio-économique des habitants ; Mamoudzou et
Cayenne restent écartées pour la raison distincte posée au batch 6. Aucune des neuf villes de ce batch
n'en relevait : la plus basse sur les écoles est Chaumont à 5,5/10 et la plus basse sur la sécurité
Vienne à 5,7/10.
...
aucune régression de routes, donc run de corpus. Compteurs mesurés avant/après : **FR 1 080, EN 845
→ 854.** Écart de corpus ramené de 235 à 226.

**La série `single-parent-in-` était rouverte depuis le 04/09 et le ROADMAP disait le contraire.**
La ligne F58 annonçait la « neuvième réouverture refermée » après le batch 8 FR ; le diff des deux
corpus donne **75 FR contre 66 EN**, et les 9 manquantes sont exactement les villes de ce batch 8
(Fontainebleau, Soissons, Vienne 38, Villefranche-sur-Saône, Auch, Chaumont, Compiègne, Les
Sables-d'Olonne, Sens). C'est le mode de régression décrit plus haut dans cette section, et il vient
de se vérifier une fois de plus : **une série « fermée » ne le reste pas, et un compteur récité ne
vaut pas un compteur mesuré.** Le run a donc écrit les 9 jumelles. Série refermée à **75/75**, dixième
réouverture.

⚠️ **Une erreur de comptage du guide FR Sens, corrigée des deux côtés.** Le guide affirmait
qu'aucun de ses quatre axes ne descend sous 6,4/10 « ce qui n'est le cas que de **deux** autres
communes des vingt premières ». Le décompte réel sur le moteur en donne **trois** : La Roche-sur-Yon,
Dole et Laon (min. d'axe 7,0 · 6,4 · 6,4). Le FR est corrigé et nomme désormais les trois, la
jumelle EN publie le même chiffre. C'est exactement le genre d'écart que ni `tsc` ni
`npm run integrity` ne peuvent voir — un décompte, pas une date — et le troisième du même genre
relevé sur cette série.

**Contrôle mécanique des figures : 1 409 nombres du texte EN cherchés dans les jumelles FR** après
normalisation des séparateurs (virgule décimale française → point, espaces fines de milliers
supprimées). **Trois seulement ne s'y trouvent pas, et les trois sont délibérés — ne pas les
« corriger » au prochain run** : ① `20 000`, le seuil d'éligibilité du classement, que seul le guide
FR Fontainebleau énonce alors que les neuf guides EN le posent en intro (un lecteur étranger ne
devinera pas ce que « les 363 communes retenues » recouvre) ; ② `11` et `1918` dans le guide
Compiègne, l'armistice signé dans un wagon **en forêt de Compiègne** — c'est ce que le nom veut dire
à l'étranger avant de vouloir dire autre chose, et la formulation dit « forêt » et non « commune »,
la convention « accessible depuis » ne permettant pas d'affirmer l'attribution communale sans
source ; ③ `1174` dans le guide Sens, l'incendie de Cantorbéry, **déjà porté par notre propre corpus
EN** (`things-to-do-in-sens-2026`, batch 37) : Guillaume de Sens est appelé à reconstruire le chœur
de Cantorbéry, filiation qui parle à un lecteur britannique et à personne d'autre.

**Autre matière propre à l'angle anglophone, sans aucun chiffre neuf** : la **désambiguïsation de
Vienne** posée en première ligne (ni Vienne en Autriche, que le français écrit pareil, ni le
département de la Vienne dont la préfecture est Poitiers), même traitement qu'Orange au batch 37,
Vernon au 39 et Bergerac au 41 ; Île-de-France et Hauts-de-Seine définis comme région et département
plutôt que supposés connus ; **T1/T2/T3 comptés en pièces et non en chambres** ; `périscolaire`
nommé et donné pour ce qu'il est, un service **communal** et non scolaire ; la caution ou garantie
exigée en plus du test de revenu, et le fait qu'un dossier sans bulletins de salaire français passe
après les autres ; `sous-préfecture` présentée comme un statut administratif et non comme une
taille ; le Vendée Globe pour Les Sables et le Beaujolais pour Villefranche en simple repère.

**Conventions du site tenues telles quelles, à ne pas diluer** : la règle des 33 % relâchée à 35 %
sous un score de coût < 5 est expliquée dans chaque guide, avec le sens de l'écart — **Fontainebleau
affiche le calcul indulgent** (3 450 € ; 3 650 € en règle stricte) et les huit autres le prudent ;
le mètre carré est **un repère éditorial tous biens confondus, pas une médiane DVF** ; l'axe écoles
décrit **l'offre communale, pas la réussite des élèves ni le travail des enseignants** ; et **aucun
verdict de sécurité par quartier** n'est publié, `data/neighborhoods.ts` ne documentant aucun
quartier des neuf villes — les guides le disent au lieu de laisser croire à une prudence de style.
Le seed et le recensement coexistent et l'écart est nommé là où il change la lecture : Fontainebleau
est **4 213 habitants sous le seuil** au recensement (15 787 contre les 25 000 du seed), Vienne,
Compiègne et Sens portent le leur en une phrase.

`metaTitle` 49-55 caractères, `metaDesc` 142-160, 6 sections par guide (comme les jumelles FR de ce
lot), 1 272-1 461 mots, **0 em-dash** sur les neuf réunis (cible R7.10 : ~1 pour 200 mots). Aucun tag
neuf : les 9 réutilisent `ile-de-france`, `hauts-de-france`, `auvergne-rhone-alpes`, `occitanie`,
`grand-est`, `pays-de-la-loire`, `bourgogne-franche-comte` — `search-index.en.json` reste à **114
tags**, donc aucune page `/tags/` créée, et `sitemap:check` confirme EN 28 737 → **28 746 URL**, soit
exactement les 9 guides neufs.

**Les 9 guides sont vérifiés retrouvés par la recherche inverse `relatedCities` de `CityGuidesList`
sur leur page ville EN et pourvus de leur photo d'en-tête** (`guideCityPhoto`). Sept sortent en 1re
position ; **Villefranche-sur-Saône et Compiègne sortent 2e, et ce n'est pas un défaut** : le tri est
par `updatedAt` décroissant, et un guide frère du même lot cite la même ville (Vienne cite
Villefranche, Soissons cite Compiègne), donc l'égalité de date se départage par l'ordre du fichier.
Les jumelles FR font pareil et moins bien — Villefranche y sort 3e et Compiègne 2e. Vérifié avant de
conclure, plutôt que corrigé en retirant des liens croisés utiles.

⚠️ **`npm run build` n'a pas été lancé, volontairement** (cf. § Commands depuis le batch 27 : 4 h 30
de génération, `.next` à 25 Go, ENOSPC avant la finalisation, aucun signal utile). Le substitut
prescrit passe en entier : `npx tsc --noEmit` **propre**, `npm run integrity` (guides EN 845 → 854),
`search-index` + `search-index:check`, `sitemap:check`, `npm run parity`, `npm run hreflang:check`,
plus le contrôle de lookup / photo et le contrôle de figures ci-dessus. ⚠️ **Note d'outillage
trouvée ce run : ne pas lancer `npx prettier --write data/guides-en.ts`.** Le fichier n'est pas au
format de la config courante, donc un passage de prettier reformate **tout le corpus** (mesuré :
9 615 insertions / 2 573 suppressions pour 9 guides ajoutés) et rend le diff illisible sur un dépôt
où ~20 agents écrivent en parallèle. Émettre le bloc au style du fichier et vérifier que le diff est
une **addition pure** (ici 441 lignes, 0 suppression). Note d'environnement reconfirmée : le
conteneur de routine démarre **sans `node_modules`** — `npm install` d'abord.

**Prochain run.** Les deux plus gros écarts de série mesurés ce run sont la série tourisme
(`10-choses-a-faire` 247 FR contre `things-to-do-in` 240 EN, rouverte par le batch 42 FR du 05/09)
et `vacances-celibataire-` (36 FR contre `solo-travel-in` 29 EN, rouverte par le batch 5 FR du
05/09) ; ⚠️ la série tourisme est traitée le jour même par son propre agent depuis le batch 39, donc
**la mesurer avant de s'y mettre** plutôt que de doubler son travail. Restent aussi `vivre-en`
(56 FR / 45 `living-in`) et `meilleures-villes` (52 FR / 41 `best-french-cities`). Les deux clusters
de quasi-doublons EN relevés le 05/09 (retraite, achat immobilier national) attendent toujours un
run de dédup, qui demande de poser les 301.
