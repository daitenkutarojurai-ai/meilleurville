...
**Phase 4 — long tail**
- [ ] EN guides — in progress (442 native EN guides via `data/guides-en.ts`; FR has 672; not 1:1 translations, native expat-angle content). 2026-06-03: **+10 native cost-of-living city guides** `cost-of-living-[city]-2026` (Paris/Lyon/Bordeaux/Toulouse/Montpellier/Nice/Nantes/Rennes/Strasbourg/Lille), grounded in the FR `budget-mensuel-realiste` figures + `housing.ts`, filling the EN budget gap (was 16 vs FR 134). Auto-registered via `EN_GUIDES`.
  - ✅ **Duplicate-slug bug FIXED (2026-06-03):** 3 slugs were declared twice (`strasbourg-living-guide-for-expats-2026`, `rennes-living-guide-for-expats-2026`, `french-school-system-expat-guide-2026`) — the 2nd of each was dead (`getEnGuide` `.find()` returns the first) + a dup sitemap URL. Kept the richer variant of each (Strasbourg/Rennes now serve the 6-section version at the unchanged URL). EN guides 445→442, zero dup slugs. A runtime dup-slug guard would prevent regressions — consider a build assertion in `data/guides-en.ts` or a test.
- 2026-06-01/02:
