...
**Phase 4 — long tail**
- [ ] EN guides — in progress (470 native EN guides via `data/guides-en.ts`; FR has 669; not 1:1 translations, native expat-angle content). **2026-06-03 per-city batches** (all grounded in the matching FR guide's real figures + `housing.ts`, auto-registered via `EN_GUIDES`):
  ...
  - Method for all: extract FR figures → parallel read-only agents → validate (schema/€-glyph/figure presence/slug-uniqueness) → single-writer insert. The `assertUniqueSlugs` guard now blocks any accidental dup at build. Remaining FR series with no EN per-city port: more `acheter`/`vivre-sans-voiture` cities, `quitter-[ville]`, `vivre-en-[region]`, comparatifs.
