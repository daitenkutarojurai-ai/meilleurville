...
**Phase 4 — long tail**
- [ ] EN guides — in progress (482 native EN guides via `data/guides-en.ts`; FR has 669; not 1:1 translations, native expat-angle content). **2026-06-03 per-city / per-region batches** (all grounded in the matching FR guide's real figures + `housing.ts`, auto-registered via `EN_GUIDES`):
  ...
  - Method for all: extract FR figures → parallel read-only agents → validate (schema/€-glyph/figure presence/slug-uniqueness) → single-writer insert. The `assertUniqueSlugs` guard blocks any accidental dup at build. Remaining FR series with no EN port: more `acheter`/`vivre-sans-voiture` cities, more `vivre-en-[region]` (Corse done as city-level, Périgord/Ariège/Creuse/Berry/Poitou etc. uncovered), comparatifs (mostly covered by `/compare`).
