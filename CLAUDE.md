...
Announced next theme (**décembre 2026**) : **la structure par âge réelle** (les sept tranches Insee de `data/city-population.json`, mêmes 361 communes éligibles) — part des 60 ans et plus de **5,3 %** à Saint-Laurent-du-Maroni à **48,7 %** aux Sables-d'Olonne, médiane **24,1 %** (mesuré ce run, la donnée est en place). Honour it or update the November guide's last section if the theme changes. Method: rank from the real pipeline score via a scratch `npx tsx` importing `@/data/cities-seed`, never from a regex read of the seed.
