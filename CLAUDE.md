...
- **R13.1 Badge embarcable "1ère/2e/Nème ville de France"** — free embeddable badge (SVG/PNG + optional `<script>` snippet) that mairies, offices de tourisme, and real-estate agents can paste on their own site to display a city's rank/score, linking back to its `/villes/[slug]` page. Grew out of the 2026-07 press/mairie outreach campaign (`[[press-outreach-log]]`): far easier "yes" for a mairie comms team than a journalist pitch, and every embed is a live backlink from a `.fr`/`.com` domain outside our own — compounds domain authority faster than one-off press hits. Scope: a route generating the badge asset per city (e.g. `/badge/[slug].svg` or an API-Worker route, mirroring the `opengraph-image.tsx` pattern) + a static landing/instructions page per city or a single `/badge` hub page explaining how to embed it. Computed entirely from existing seed scores — no new dataset.
  - **⚠️ Whichever route shape is chosen, add it to `app/sitemap.ts` (new chunk or existing `city-sub` chunk) and bump `SITEMAP_CHUNK_COUNT` awareness in `robots.ts` stays automatic (it derives from `SITEMAP_CHUNKS.length`) — but don't forget the actual route.** Sitemap chunks are order-stable and enumerated explicitly in `robots.txt` (`Sitemap: .../sitemap/0.xml` … `/16.xml` today); a new content route that's SSG'd but never added to a chunk is invisible to Search Console even though it builds fine. If the badge is served dynamically from the Worker (not SSG'd via `output: "export"`), it doesn't need a sitemap entry itself, but the per-city instructions/landing page (if any) does.
