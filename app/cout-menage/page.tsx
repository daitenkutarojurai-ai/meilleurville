  ...
  return (
    <main id="main-content" className="min-h-screen">
      ...
      <section className="mx-auto max-w-5xl px-4 py-10">
        ...
        <h2 className="mt-10 text-xl font-semibold text-[var(--text-primary)]">Top 12 villes les moins chères — famille 2 enfants</h2>
              ...
              {topCheapFamille.map((r, i) => (
                <tr key={r.citySlug} className="border-b border-[var(--border)] hover:bg-[var(--bg-surface)]">
                  ...
                  <td className="px-3 py-2 text-sm">
                    <Link href={`/cout-menage/${r.citySlug}`} className="text-[var(--text-primary)] font-medium hover:underline">
                      {r.cityName}
                    </Link>
                  </td>
                  <td className="px-3 py-2 text-sm text-[var(--text-secondary)]">{r.region ?? "—"}</td>
        ...
        {/* Top 12 solo */}
        <h2 className="mt-10 text-xl font-semibold text-[var(--text-primary)]">Top 12 villes les moins chères — solo</h2>
        <p className="mt-1 text-xs text-[var(--text-tertiary)]">T1 + transit prioritaire. Tri par total mensuel croissant.</p>
        <Card className="mt-3 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-[var(--bg-surface)] text-xs uppercase tracking-wide text-[var(--text-tertiary)]">
              <tr>
        ...
        <h2 className="mt-10 text-xl font-semibold text-[var(--text-primary)]">8 villes les plus chères pour un·e retraité·e</h2>
        <p className="mt-1 text-xs text-[var(--text-tertiary)]">T2 sans trajet domicile-travail. Tri par total décroissant.</p>
        <Card className="mt-3 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-[var(--bg-surface)] text-xs uppercase tracking-wide text-[var(--text-tertiary)]">
              <tr>
