...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Sécurité à ${city.name} — profil estimé`,
    description: clampMeta(`Profil estimé de la sécurité à ${city.name} (${city.department}) : atteintes biens ${SAFETY_LEVEL_LABEL[s.property.level].toLowerCase()}, personnes ${SAFETY_LEVEL_LABEL[s.persons.level].toLowerCase()}, nuit ${SAFETY_LEVEL_LABEL[s.nocturnal.level].toLowerCase()}, VFFS ${SAFETY_LEVEL_LABEL[s.vffs.level].toLowerCase()}. Score ${(10 - s.composite).toFixed(1)}/10 (10 = ville la plus sûre).`),
  ...
  const faq = faqJsonLd([
    {
      ...
      a: `${city.name} (${city.department}) affiche un composite sécurité estimé de ${(10 - s.composite).toFixed(1)}/10 (10 = ville la plus sûre). Détail : atteintes biens ${(10 - s.property.score).toFixed(1)}/10, personnes ${(10 - s.persons.score).toFixed(1)}/10, nuit ${(10 - s.nocturnal.score).toFixed(1)}/10, VFFS ${(10 - s.vffs.score).toFixed(1)}/10. ${s.signature}`,
    },
    {
      q: `Où voir les vrais chiffres SSMSI pour ${city.name} ?`,
      a: `Nos quatre sous-scores sont estimés et ne reprennent aucun taux publié — pour les vrais chiffres, le SSMSI (interstats.fr) publie chaque mois les indicateurs de délinquance par département et par commune ≥ 10 000 hab. Les données ouvertes data.gouv.fr proposent le détail par taux pour 1 000 hab. L'enquête « Cadre de Vie et Sécurité » (Insee, 14 000 ménages/an) complète avec la victimation ressentie.`,
    },
    {
      q: `Comment lire les 4 sous-axes ?`,
      a: `Atteintes biens = cambriolages + vols véhicules + vols sans violence (concentré sur métropoles touristiques et IDF dense). Atteintes personnes = coups & blessures volontaires hors VFFS (concentré sur métropoles + bassins industriels en tension). Sécurité nocturne = rixes, agressions, dégradations nocturnes (concentré sur centres festifs/étudiants). VFFS = violences faites aux femmes, dimension calée sur le suivi SSMSI des signalements. Les quatre sous-axes sont estimés depuis le score sécurité de la ville, sa taille et son profil : aucun taux pour 1 000 habitants n'est ingéré.`,
    },
    {
    ...
    <main id="main-content" className="min-h-screen">
      ...
      <section className="mx-auto max-w-3xl px-4 sm:px-6 py-10">
        ...
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-[var(--text-primary)]">
          Sécurité à {city.name} — profil estimé
        </h1>
        <p className="mt-3 text-base text-[var(--text-secondary)]">
          Décomposition du score sécurité en 4 sous-axes : atteintes aux biens, atteintes
          aux personnes, sécurité nocturne, violences faites aux femmes. Ces quatre niveaux
          sont <strong>estimés</strong> depuis le score sécurité de {city.name}, sa taille et
          son profil de commune — le découpage suit les catégories du SSMSI, mais{" "}
          <strong>aucun taux pour 1 000 habitants n&apos;est repris</strong> : ce ne sont pas
          les faits enregistrés dans la commune. Les chiffres réels sont publiés par{" "}
          <a
        ...
        <Card className={`mt-6 border-l-4 ${SAFETY_LEVEL_BG[s.level].replace('bg-', 'border-l-').replace('-50', '-500').split(' ')[0]}`}>
          ...
          <p className="text-xs text-[var(--text-tertiary)] mb-2">10 = ville la plus sûre · estimation communale suivant le découpage SSMSI, pas les faits enregistrés.</p>
        ...
        <Card className="mt-3">
          <ul className="space-y-2 text-sm text-[var(--text-secondary)] leading-relaxed">
            <li>
              ...
              métropolitain × tags tourisme, sans reprendre de taux enregistré. Repère
              national SSMSI des vols sans violence, pour situer l&apos;échelle : ~16,5 ‰.
            </li>
            <li>
              <strong className="text-[var(--text-primary)]">Atteintes aux personnes (30 %) :</strong>{" "}
              coups & blessures volontaires (hors VFFS), estimés — repère national SSMSI
              ~4,3 ‰, cité pour l&apos;échelle et non relevé ici. Surcote
          ...
          <p className="text-xs text-[var(--text-tertiary)] mt-4">
            Ces quatre sous-scores sont des <strong>estimations</strong> à l&apos;échelle
            communale, dérivées du score sécurité de la ville plus des ajustements de
            département et de tags : aucun fait enregistré, aucun taux pour 1 000 habitants
            n&apos;est ingéré ici. À l&apos;adresse, l&apos;expérience varie d&apos;ailleurs
            fortement selon le quartier. Pour les chiffres réels, l&apos;état 4001 de la
            préfecture et les publications du SSMSI font foi.
          </p>
        </Card>
        ...
        <div className="mt-8 text-sm space-y-2">
          <div>
            <Link href="/securite" className="text-[var(--accent)] hover:underline">
              → Classement national de la sécurité
            </Link>
          </div>
          <div>
            <Link href="/classements/securite-femme-seule" className="text-[var(--accent)] hover:underline">
              → Le même socle, vu du retour tardif d&apos;une femme seule
            </Link>
