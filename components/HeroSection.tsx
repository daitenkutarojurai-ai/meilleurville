...
export function HeroSection() {
  ...
  return (
    <section className="relative overflow-hidden bg-[var(--bg-canvas)] bg-sun-glow pt-20 pb-24">
      {/* Soft floating glows */}
      <div className="pointer-events-none absolute inset-0" aria-hidden>
        <div className="absolute top-10 left-1/2 -translate-x-1/2 w-[800px] h-[500px] rounded-full bg-[var(--accent)]/15 blur-[140px]" />
        <div className="absolute bottom-0 right-0 w-[420px] h-[320px] rounded-full bg-[var(--accent-warm)]/15 blur-[120px]" />
        <div className="absolute top-20 left-0 w-[300px] h-[260px] rounded-full bg-[var(--accent-pink)]/8 blur-[100px]" />
      </div>

      <div className="relative mx-auto max-w-5xl px-4 sm:px-6 text-center">
        {/* Friendly intro pill */}
        <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-[var(--accent)]/30 bg-white px-4 py-2 text-sm text-[var(--accent)] shadow-sm">
          <span className="text-base leading-none">🌿</span>
          <span className="font-medium">{`${CITIES_SEED.length}+ villes · L'avis des vrais habitants`}</span>
        </div>

        {/* Headline */}
        <h1 className="mb-5 text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight leading-[1.05] text-[var(--text-primary)]">
          Là où vous serez{" "}
          <span className="gradient-text">heureux de vivre.</span>
        </h1>
        ...
        <div ref={containerRef} className="relative mb-6 mx-auto max-w-xl">
          <form onSubmit={handleSearch}>
            <div className="flex items-center gap-2 rounded-2xl border-2 border-[var(--border)] bg-white p-2 shadow-lg shadow-[var(--accent)]/10 focus-within:border-[var(--accent)]/60 focus-within:shadow-[var(--accent)]/20 transition-all">
          ...
          {open && suggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-2 z-50 rounded-2xl border border-[var(--border)] bg-white shadow-xl shadow-[var(--accent)]/15 overflow-hidden">
              {suggestions.map((r) => (
                <button
                  key={r.slug}
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-12">
          ...
          {TRENDING.map((city) => (
            <Link
              ...
              className="flex items-center gap-1 rounded-full border border-[var(--border)] bg-white px-3 py-1 text-xs text-[var(--text-secondary)] transition-all hover:border-[var(--accent)]/50 hover:text-[var(--accent)] hover:shadow-sm hover:-translate-y-0.5"
            >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link href="/quiz">
            <Button size="lg" className="gap-2 text-base shadow-xl shadow-[var(--accent)]/30 accent-pulse">
